...
import com.example.staff_service.model.Pack4RBMQ;
...
public class RabbitMQSender {
    ...
    public Result sendMessage2DriverService(Pack4RBMQ pack4RBMQ) {
    ...
    public Result sendMessage2UserService(Pack4RBMQ pack4RBMQ) {
        String routingKey="routingKey.user";
        //LOGGER.info(String.format("Json message sent -> %s", user.toString()));
        return new Gson().fromJson(String.valueOf(rabbitTemplate.convertSendAndReceive(exchange, routingKey, pack4RBMQ)),Result.class);
    }

    public Result sendMessage2DriveService(Pack4RBMQ pack4RBMQ) {
        String routingKey="routingKey.driver";
        //LOGGER.info(String.format("Json message sent -> %s", user.toString()));
        return new Gson().fromJson(String.valueOf(rabbitTemplate.convertSendAndReceive(exchange, routingKey, pack4RBMQ)),Result.class);
    }
}
