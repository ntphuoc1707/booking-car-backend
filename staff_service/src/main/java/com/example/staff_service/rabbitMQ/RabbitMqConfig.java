...
public class RabbitMqConfig {
    ...
    @Bean
    Binding binding2User(DirectExchange exchange) {
        return BindingBuilder.bind(new Queue("queue.user",false))
                .to(exchange)
                .with("routingKey.user");
    }

    @Bean
    Binding binding2Driver(DirectExchange exchange) {
        return BindingBuilder.bind(new Queue("queue.driver",false))
                .to(exchange)
                .with("routingKey.driver");
    }
